...
class QGLSTMRNNModel(object):
    ...
    def _add_variables(self):
        ...
        # embeddings layer
        self.word_embeddings_lookup = tf.get_variable('embeddings',
                                                      shape=(self.config.Vocab_len, self.config.embedd_size))

        # projection layer
        self.W = tf.Variable(tf.random_uniform([self.config.decoder_hidden_state_size, self.config.Vocab_len], -1, 1),
                             dtype=tf.float32)
